#include "p-0001_0001_0003_0006.h"

namespace f3d {
	namespace core {
		namespace prog {
			Program_0001_0001_0003_0006::Program_0001_0001_0003_0006(std::shared_ptr< f3d::core::PhysicalDevice >& phys, std::shared_ptr< f3d::core::Device >& device) : 
				Program::Program(device->vk_device, { F3D_COLOR_DIFFUSE | F3D_COLOR_SPECULAR, F3D_TEXTURE_DIFFUSE | F3D_TEXTURE_SPECULAR, F3D_LIGHT_DIRECTIONAL, F3D_SHADING_FLAT }),
				DescriptorContainer::DescriptorContainer(phys, device) {
			}

			Program_0001_0001_0003_0006::~Program_0001_0001_0003_0006() {
				std::cout << "Destructor: " << __FILE__ << std::endl;
			}

			void									Program_0001_0001_0003_0006::initVkPipeline(VkRenderPass& renderpass, uint32_t subpass) {
				VkResult							r;
				...
				F3D_ASSERT(createSpvShader("0001_0001_0003_0006.vert.spv", &vert_shader) != false, "0001_0001_0003_0006 vertex shader");
				...
				F3D_ASSERT(createSpvShader("0001_0001_0003_0006.frag.spv", &frag_shader) != false, "0001_0001_0003_0006 fragment shader");
			...
			void				Program_0001_0001_0003_0006::initVkPipelineInfos() {
			...
			void									Program_0001_0001_0003_0006::initVkLayout() 
			{
				VkResult							r;
				...
				DescriptorContainer::addDescriptorBinding(2, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_VERTEX_BIT);
				//layout (set = 2, binding = 1) uniform uint n_light;
				DescriptorContainer::addDescriptorBinding(2, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT);
				...
				DescriptorContainer::addDescriptor(5); //layout (set = 5, binding = 0) uniform sampler2D specular_samplerColor;
				DescriptorContainer::addDescriptorBinding(5, 0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT);

				vk_desc_layout = new VkDescriptorSetLayout[6];
				...
				vk_desc_layout[5] = DescriptorContainer::getDescriptorLayout(5);

				std::memset(&pipe_layout_info, 0, sizeof(VkPipelineLayoutCreateInfo));
				pipe_layout_info.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
				pipe_layout_info.pNext = NULL;
				pipe_layout_info.setLayoutCount = 6;
				pipe_layout_info.pSetLayouts = vk_desc_layout;
				r = vkCreatePipelineLayout(vk_device, &pipe_layout_info, NULL, &vk_pipeline_layout);
				F3D_ASSERT_VK(r, VK_SUCCESS, "Program_0001_0001_0003_0006 Create pipeline layout failed");
			}

			bool							Program_0001_0001_0003_0006::drawToCommandBuffer(VkCommandBuffer& cmd, f3d::tree::Mesh& mesh, f3d::tree::Scene& scene)
			{
				...
				f3d::tree::TextureImpl*		specular_texture = dynamic_cast<f3d::tree::TextureImpl *>(material->getTexture(F3D_TEXTURE_SPECULAR));
				VkBuffer					vertex_bufs[3] = { m.getVertexBuffer(), m.getNormalBuffer(), m.getUvBuffer() };
				VkDeviceSize				vertex_offsets[3] = { 0, 0, 0 };				
				VkDescriptorSet				sets[6] = { cam.getDescriptorSet() , m.getDescriptorSet(), 
														sc.getLightsDescriptorSet(), material->getDescriptorSet(), 
														diffuse_texture->getDescriptorSet(), specular_texture->getDescriptorSet() };

				vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, vk_pipeline_layout, 0, 6, sets, 0, nullptr);
				Program::bind(cmd);
