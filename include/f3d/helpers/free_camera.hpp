#ifndef _F3D_HELPER_FREE_CAMERA_HPP
#define _F3D_HELPER_FREE_CAMERA_HPP
...
namespace f3d {
	namespace helpers {
		...
		public:
			...
			FreeCamera(const FreeCamera& oth) = delete; //No copy constructor
			FreeCamera(FreeCamera&& mov) = delete; // No move constructor
			FreeCamera&	operator=(const FreeCamera& oth) = delete; //No copy assignement
			FreeCamera&	operator=(FreeCamera&& mov) = delete; //No mov assignement
			...
			void				updateFromMouse(f3d::helpers::Mouse& mouse) 
			{
				...
				{
					x -= static_cast<float>(mouse.getDeltaX()) / 80.0f;
			...
			void					updateFromXbox360(f3d::helpers::Xbox360Controller& joystick) {
				...
				ay += joystick.axisState(joystick.AXIS_RS_X) / 60.0f;
